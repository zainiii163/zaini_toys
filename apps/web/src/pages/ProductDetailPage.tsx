    ...
    <div className="container-toy py-8">
            ...
            <div className="mt-3 flex gap-2 overflow-x-auto pb-2">
              {images.map((img: any, i: number) => (
                <button
                  ...
                  className={`flex-shrink-0 h-20 w-20 rounded-lg border-2 overflow-hidden ${
                    i === galleryIdx ? 'border-blue-500' : 'border-transparent hover:border-gray-300'
                  }`}
                >
        ...
        <div>
          ...
          <div className="mt-2 flex items-center gap-3">
            ...
            {product.isNewArrival && (
              <span className="rounded bg-teal-50 px-2 py-0.5 text-xs font-bold text-teal-600">
                New
              </span>
            )}
          </div>
          ...
          <div className="mt-4 flex items-center gap-3">
            <button onClick={onAddWish} className="flex items-center gap-1 text-gray-600 hover:text-red-500">
        ...
        <div className="py-6">
          {activeTab === 'details' && (
            <div className="prose max-w-none">
              ...
              {product.educationalBenefits?.length && (
                <>
              ...
              {summaryData?.data && (
                <div className="mb-6 grid gap-4 sm:grid-cols-2">
                  <div className="rounded-xl bg-gray-50 p-6">
                    <div className="flex items-baseline gap-2">
                      ...
                      <div>
                        ...
                        <span className="text-gray-500">({summaryData.data.total || 0} reviews)</span>
                      </div>
                    </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <Link to={`/product/${slug}#reviews`} className="text-sm text-blue-600 hover:underline">
                      View all reviews
                    </Link>
                  </div>
                </div>
      ...
      <section className="mt-12">
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
          {relatedData?.data?.map((p: any) => (
            <Link key={p._id} to={`/product/${p.slug}`} className="card-toy group overflow-hidden">
              <img src={p.images?.[0]?.url} alt={p.name} className="aspect-square w-full object-cover group-hover:scale-105 transition-transform" loading="lazy" />
              <div className="p-3">
                <p className="line-clamp-1 text-sm font-medium">{p.name}</p>
                <p className="text-sm font-bold text-gray-900">Rs. {(p.salePrice ?? p.price).toLocaleString()}</p>
              </div>
            </Link>
          ))}
        </div>
